...
int main(int argc, char **argv) {
    ...
    for (i = 0; i < input_group_num; i++) {
        ...
        for (int j = 0; j < input_num; j++) {
            ...
            sess0_input[j] = shl_ref_f32_to_input_dtype(j, sess0_inputf[j], sess0);
        }
        memcpy(sess0_input_aligned[0], sess0_input[0], input_size[0]);
        ...
        FILE *file = fopen(argv[3], "rb"); // 打开二进制文件
        if (file == NULL) {
