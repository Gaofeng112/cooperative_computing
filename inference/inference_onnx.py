...
output_image_path = "./output/origin"
...
for index, timestep in progbar:
    ...
    initial_input_data = {
        "sample": np.array(latent, dtype=np.float32).transpose(0, 3, 1, 2),#(1, 4, 32, 32)
        "timestep": np.array([float(timestep)], dtype=np.float32),#(1,)
        "encoder_hidden_states": np.array(context, dtype=np.float32),#(1, 77, 768)
        # "pixel_values": np.random.rand(1, 3, 768, 768).astype(np.float32)#765 766 767
    }
    e_t_hf = model_inference.inference(initial_input_data)
...
image.save(os.path.join(output_image_path, args.output))
