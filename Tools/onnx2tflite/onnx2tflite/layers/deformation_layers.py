    ...
    def __call__(self, inputs):
        ...
        def calculate_reshape_shape(input_shape, target_shape):
            ...
            if -1 in target_shape:
                ...
                # 如果没有动态维度，则直接返回目标形状
                return target_shape
        final_shape = calculate_reshape_shape(inputs.shape, self.out_shape)
        print("Final shape:", final_shape)
        inputs = tf.reshape(inputs, shape=final_shape)
        return inputs
...
class TFShape():
    def __init__(self, tensor_grap, node_weights, node_inputs, node_attribute, node_outputs, layout_dict, *args, **kwargs):
        super().__init__()
        # 因为Shape节点的作用是获取tensor的shape，不需要处理out_shape或trans_in
        self.trans_in = None
        for nop in node_outputs:
            layout_dict[nop] = Layout.Channel_First

    def __call__(self, inputs):
        # 直接获取输入tensor的shape并返回
