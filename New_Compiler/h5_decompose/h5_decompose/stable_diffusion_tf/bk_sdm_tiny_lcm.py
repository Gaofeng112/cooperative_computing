...
from .diffusion_model import BkTinyLCMUNetModel
...
class BKSDTinyLCM:
    ...
    def generate(
        ...
        # print("the timestep is ", list(enumerate(timesteps))[::-1])
    ...
    def get_latent_context(self, prompt, negative_prompt=None, batch_size=1, seed=None, num_steps=25, temperature=1, unconditional_guidance_scale=7.5):
        inputs = self.tokenizer.encode(prompt)
        assert len(inputs) < 77, "Prompt is too long (should be < 77 tokens)"
        phrase = inputs + [49407] * (77 - len(inputs))
        phrase = np.array(phrase)[None].astype("int32")
        ...
        latent, alphas, alphas_prev = self.get_starting_parameters(
            timesteps, batch_size, seed , input_image=None, input_img_noise_t=input_img_noise_t
        )
        progbar = tqdm(list(enumerate(timesteps))[::-1])
        for index, timestep in progbar:
            return latent, timestep, context, unconditional_context

    def get_single_output(self, latent, t, context, unconditional_context, unconditional_guidance_scale, batch_size):
        timesteps = np.array([t])
        t_emb = self.timestep_embedding(timesteps)
        t_emb = np.repeat(t_emb, batch_size, axis=0)
        print("the shape of latent is ", latent.shape)
        print("the shape of t_emb is ", t_emb.shape)
        print("the shape of context is ", context.shape)
        unconditional_latent = self.diffusion_model.predict_on_batch(
            [latent, t_emb, unconditional_context]
        )
        latent = self.diffusion_model.predict_on_batch([latent, t_emb, context])
        return latent, unconditional_latent
    ...
    # for time condtioning embedding
    def get_guidance_scale_embedding(self, w, embedding_dim=512, dtype=tf.float32):
        w = w * 1000.0

        half_dim = embedding_dim // 2
        emb = tf.math.log(10000.0) / (half_dim - 1)
        emb = tf.exp(tf.range(half_dim, dtype=dtype) * -emb)
        emb = tf.expand_dims(w, axis=-1) * tf.expand_dims(emb, axis=0)
    ...
    def get_model_output(
        ...
        t_emb = self.timestep_embedding(timesteps)
        t_emb = np.repeat(t_emb, batch_size, axis=0)
        t_cond = self.get_guidance_scale_embedding(unconditional_guidance_scale - 1, 256)
        unconditional_latent = self.diffusion_model.predict_on_batch(
            [latent, t_emb, t_cond, unconditional_context]
        )
        latent = self.diffusion_model.predict_on_batch([latent, t_emb, t_cond, context])
...
def get_models(img_height, img_width, download_weights=True):
    ...
    t_cond = keras.layers.Input((256,))
    latent = keras.layers.Input((n_h, n_w, 4))
    unet = BkTinyLCMUNetModel()
    diffusion_model = keras.models.Model(
        [latent, t_emb, t_cond, context], unet([latent, t_emb, t_cond, context])
    )
