...
# load model
onnx_model = onnx.load("net/diffusion_model_fp32.onnx")
...
input_tensor_new = onnx.helper.make_tensor_value_info(name = input_tensor.name, elem_type = 1, 
                                                      shape = [1, input_shape[1].dim_value])
graph.input.remove(input_tensor)
graph.input.insert(0, input_tensor_new)

input_tensor = graph.input[1]
input_shape = input_tensor.type.tensor_type.shape.dim
input_tensor_new = onnx.helper.make_tensor_value_info(name = input_tensor.name, elem_type = 1, 
                                                      shape = [1, input_shape[1].dim_value])
graph.input.remove(input_tensor)
graph.input.insert(1, input_tensor_new)

input_tensor = graph.input[2]
input_shape = input_tensor.type.tensor_type.shape.dim
input_tensor_new = onnx.helper.make_tensor_value_info(name = input_tensor.name, elem_type = 1, 
                                                      shape = [1, input_shape[1].dim_value, input_shape[2].dim_value, input_shape[3].dim_value])
graph.input.remove(input_tensor)
graph.input.insert(2, input_tensor_new)

input_tensor = graph.input[3]
input_shape = input_tensor.type.tensor_type.shape.dim
input_tensor_new = onnx.helper.make_tensor_value_info(name = input_tensor.name, elem_type = 1, 
                                                      shape = [1, input_shape[1].dim_value, input_shape[2].dim_value])
graph.input.remove(input_tensor)
graph.input.insert(3, input_tensor_new)

output_tensor = graph.output[0]
output_shape = output_tensor.type.tensor_type.shape.dim
output_tensor_new = onnx.helper.make_tensor_value_info(name = output_tensor.name, elem_type = 1, 
                                                      shape = [1, output_shape[1].dim_value, output_shape[2].dim_value, output_shape[3].dim_value])
graph.output.remove(output_tensor)
graph.output.insert(0, output_tensor_new)
...
onnx.save(inferred_onnx_model,"./net/diffusion_model_fp32_new.onnx")
